...
# # 链接选项（加入glib相关库）
# LIBS = -lssl -lcrypto -lcjson -lmysqlclient -lncurses $(shell pkg-config --libs glib-2.0) -lglib-2.0 -I/path/to/glib/include -L/path/to/glib/lib -lglib-2.0
...
# # 规则：生成服务器可执行文件
# $(SERVER_TARGET): $(SERVER_OBJS) $(COMMON_OBJS) | $(BIN_DIR)
# 	$(CC) $(CFLAGS) -o $@ $^ $(LIBS) -I/path/to/glib/include -L/path/to/glib/lib -lglib-2.0
...
# # 规则：编译 server/ 目录下的 .c 文件生成 .o 文件
# $(OBJ_DIR)/%.o: $(SERVER_DIR)/%.c | $(OBJ_DIR)
# 	$(CC) $(CFLAGS) $(INCLUDES) -c $< -o $@ -I/path/to/glib/include -L/path/to/glib/lib -lglib-2.0
